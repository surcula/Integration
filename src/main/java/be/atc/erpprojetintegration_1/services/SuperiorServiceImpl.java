...
public class SuperiorServiceImpl implements ISuperiorService {
    ...
    /**
     * Recupere le superieur actif lie a un employe.
     *
     * @param employeeId identifiant de l'employe supervise
     * @return resultat contenant la relation active
     */
    @Override
    public Result<Superior> getActiveByEmployeeId(Integer employeeId) {
        ...
        try {
            Superior superior = em
                    .createNamedQuery("getActiveSuperiorByEmployeeId", Superior.class)
            ...
            Map<String, String> errors = new HashMap<>();
            errors.put("notFound", "evaluations.error.superior.notFound");
            log.warn("No active superior found for employee id: " + employeeId);
            return Result.fail(errors);

        } catch (Exception ex) {
            Map<String, String> errors = new HashMap<>();
            errors.put("message", "superiors.error.load");
            log.error("Error while searching active superior for employee id: " + employeeId, ex);
            return Result.fail(errors);
